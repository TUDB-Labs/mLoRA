...
class CPOTask(TrainTask):
    ...
    def data(self, start_idx: int) -> Tuple[List[Tokens], List[MLoRADataConfig]]:
        ...
        def loss_fn(input: torch.Tensor, target: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
            ...
            batch_label = target[start_idx:end_idx,
                                 1:].contiguous().to(input.device)
            ...
            loss_chosen: torch.Tensor = F.cross_entropy(
                chose_input, chose_label)

            # step2. calc the prefer loss
            logits = batch_input.log_softmax(-1)
            per_token_logps = torch.gather(
                logits, dim=2, index=batch_label.unsqueeze(2)).squeeze(2)
